...
// Yesterday (so you can still note a call that just ended) through +14 days.
export async function listEvents() {
  const timeMin = new Date(Date.now() - 24 * 3600e3).toISOString();
  const timeMax = new Date(Date.now() + 14 * 86400e3).toISOString();
