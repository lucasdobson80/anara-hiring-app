...
// Researcher scoring — separate track (Laia's pharma/biotech/CRO hiring).
// Same shape, but niche is the free-text role slug rather than a creator niche.
const RESEARCH_SCORE_SCHEMA = {
  type: "object",
  properties: {
    scores: {
      type: "array",
...
export async function scoreCandidates(candidates, threshold = 73, { ugcMode = false, researcher = false } = {}) {
  ...
  const system = researcher
    ? RESEARCHER_PROMPT.replaceAll("{{THRESHOLD}}", String(threshold))
    : (ICP_PROMPT + (ugcMode ? UGC_MODE_ADDENDUM : "")).replaceAll("{{THRESHOLD}}", String(threshold));
  const schema = researcher ? RESEARCH_SCORE_SCHEMA : SCORE_SCHEMA;
  const block = researcher ? researcherBlock : candidateBlock;
