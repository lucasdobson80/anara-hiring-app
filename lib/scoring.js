...
- A skilled on-camera lifestyle/tech/student creator with a decent ratio (around 1x or better) should clear the bar even with zero study angle.
- Mid-size accounts (1k-60k followers) are the sweet spot: big influencers rarely join a pay-per-video program, so do NOT reward size — reward skill at any size within range.
...
export async function scoreCandidates(candidates, threshold = 73) {
  ...
  for (let i = 0; i < candidates.length; i += CHUNK) {
    ...
    const response = await anthropic.messages.create({
      model: SCORING_MODEL,
      max_tokens: 8000,
      system: ICP_PROMPT.replaceAll("{{THRESHOLD}}", String(threshold)),
