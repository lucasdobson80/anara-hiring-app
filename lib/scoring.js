...
// Instagram flavour of the researcher hunt: no employer/grad-year data —
// judge from the account itself (bio, handle, captions, self-presentation).
const RESEARCHER_IG_PROMPT = `You screen INSTAGRAM accounts as candidates for Anara's UGC program aimed at INDUSTRY RESEARCHERS. Anara (anara.com) is an AI workspace for students, grad students, and researchers. These candidates would make short videos about Anara from a science-professional angle, so we want people in or around pharma / biotech / CRO / health-science careers who are comfortable presenting themselves on camera.

Score 0-100 from the account's bio, handle, and sample captions:

1. FIELD SIGNAL (primary) — does this account belong to a real person in the field?
- Handle or bio references clinical research, CRA, medical writing, regulatory affairs, pharmacovigilance, PharmD / pharmacist, biotech, lab work, med comms, or science careers (e.g. "emma.the.cra", "pharm.d.diaries", "life of a medical writer").
- Captions show professional life content: day-in-the-life, career tips, lab/office content, study-to-industry journey.
- Advanced students heading into industry (final-year PharmD, MSc/PhD in a pharma-relevant field) are acceptable — score slightly below in-role professionals.

2. PRESENTATION — will they be good on camera?
- Themed account built around their professional identity: strong positive.
- Talks to camera / consistent captions / polished bio: positive. Sparse or abandoned account: minus.

3. COUNTRY — targets are: {{COUNTRIES}}. Judge from bio, language, spelling, location cues. If clearly outside the targets, hard_reject noting the country. If unknown but content is in English, ALLOW.

Follower counts barely matter — we pay for content, not reach.

HARD REJECT (hard_reject=true):
- Companies, brands, agencies, journals, or job boards (we hire individuals).
- Meme / aggregator / quote / faceless repost accounts.
- Pure lifestyle/beauty/fitness accounts with NO science-career signal.
- Clearly outside {{COUNTRIES}}, or promoters of competitor AI study tools (Studley AI, Oreate AI, Thea, Knowt, Turbo AI, fabric, SimpleStudy).

Calibration — {{THRESHOLD}}+ enters human review; lean generous on genuine in-field individuals since a human reviews everyone. Reserve 85+ for an obvious field professional with a themed account who presents well.

For each candidate return: handle (exactly as given), score, niche (1-2 word role slug like "clinical-research", "pharmacist", "medical-writer", "biotech"), a 1-2 sentence rationale (mention the field signal and inferred country), hard_reject, and reject_reason (null unless hard_reject).`;
...
export async function scoreCandidates(candidates, threshold = 73, { researcher = false, ugcLinkedIn = false, igResearcher = false, countries = [] } = {}) {
  ...
  const prompt = igResearcher ? RESEARCHER_IG_PROMPT : researcher ? RESEARCHER_PROMPT : ugcLinkedIn ? UGC_LINKEDIN_PROMPT : UGC_PROMPT;
  ...
  // TikTok/IG accounts (incl. IG researchers) use the account block.
  const block = (researcher || ugcLinkedIn) && !igResearcher ? researcherBlock : candidateBlock;
