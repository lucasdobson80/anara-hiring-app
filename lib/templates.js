...
export const STAGES = ["New", "Approved", "Rejected", "Contacted", "Replied", "Interview", "Trial", "Signed"];
export const ONBOARD_STAGES = ["Approved", "Contacted", "Replied", "Interview", "Trial", "Signed"];
...
export function dmTemplate(first) {
  const greet = first ? `Hey ${first}!` : "Hey!";
  return `${greet} I'm XX and I'm running Growth for Anara. Our website helps grad students and researchers find, understand and write papers faster.
We have a UGC program where creators post videos on new accounts solely about Anara, like @kcpagess (example video https://vm.tiktok.com/ZNR3cDs5M/). Creators earn from $25 for every video they make and post, and bonuses when videos go viral :)
We normally work on 10–20 short pieces of content/week (flexible), making it $1–2k fixed per month + bonuses!
I came across your account and really loved your content. Let me know if you'd be interested — I can send more details or we can have a quick call to discuss further.`;
}

// Email outreach reuses the company-approved DM wording verbatim (same
// offer, same example) with a subject line — a warmer second channel
// alongside the manual TikTok DM.
export function outreachEmail(first) {
  return { subject: "Paid UGC opportunity with Anara", body: dmTemplate(first) };
}

// Opens Gmail's web compose directly (Lucas's preference over the OS mail
// app) with recipient, subject, and the approved copy pre-filled.
export function mailtoLink(email, first) {
  const { subject, body } = outreachEmail(first);
